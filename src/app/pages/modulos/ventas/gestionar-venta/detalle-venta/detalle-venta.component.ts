...
export class DetalleVentaComponent implements OnInit {
  ...
  crearFormCuotas() {
    ...
    this.formCuotas = this.formBuilder.group({
      precioCuota: [null, [Validators.required, Validators.min(precioTotal), Validators.max(precioTotal)]],
      fechaCuota: [null, [Validators.required]],
      pagoCuota: [false],
    })

    this.formCuotas.get('pagoCuota').valueChanges.subscribe(
      data => {
        this.tipoCuota(data)
      }
    )
  ...
  async listarProyectos() {
    try {
      ...
      this._obvsService.toogleSpinner();
      const data: any = await this._proyectoService.getProyectosPaginado().toPromise()
  ...
  async registrarCuota() {
    ...
    if (this.formProyectos.invalid && this.tipoVenta == 'Proyecto' && !this.segundaCuota) {
    ...
    if (this.formPedidos.invalid && this.tipoVenta == 'Producto' && !this.segundaCuota) {
  ...
  tipoCuota(data) {
    ...
    if (Boolean(data)) {
      ...
      this.formCuotas.controls.precioCuota.setValidators([Validators.required, Validators.min(precioTotal / 2), Validators.max(precioTotal - 1)])
      this.formCuotas.controls.precioCuota.updateValueAndValidity()
    }
    else {

      this.formCuotas.controls.precioCuota.setValidators([Validators.required, Validators.min(precioTotal), Validators.max(precioTotal)])
      this.formCuotas.controls.precioCuota.updateValueAndValidity()
    }
  }
